...
- `pesaguard_backend_pipeline/scripts/backup_postgres.sh` writes locally by default; optional `S3_BUCKET` upload must use a bucket in `BACKUP_REGION`.
...
- `pesaguard_backend_pipeline/tenant_settings.json` — per-tenant `deployment_region`, `backup_region`, `log_region`
