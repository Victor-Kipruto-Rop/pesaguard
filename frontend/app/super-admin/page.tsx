...
interface SummaryPayload {
  ...
  open_discrepancies_total?: number;
...
const initialTrend = [{ date: 'Now', value: 0 }];
...
export default function AdminDashboardPage() {
  ...
  const [trendData, setTrendData] = useState<{ date: string; value: number }[]>(initialTrend);
  const [discrepancies, setDiscrepancies] = useState<DiscrepancyFeedItem[]>([]);
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        const summaryRes = await adminFetch<SummaryPayload>('/api/stats/summary');
        const feedRes = await adminFetch<{ results: DiscrepancyFeedItem[] }>('/api/discrepancies?limit=6');

        if (summaryRes.ok && summaryRes.data) {
          setSummary(summaryRes.data);
          setError('');
          setTrendData((previous) => {
            const rate = summaryRes.data?.reconciliation_rate != null ? summaryRes.data.reconciliation_rate * 100 : 0;
            return [...previous.slice(-7), { date: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), value: Number(rate.toFixed(2)) }];
          });
        } else if (summaryRes.status === 403) {
          setError('Admin access token is invalid. Update it in Settings to continue.');
        } else {
          setError('Unable to load reconciliation summary metrics.');
        }

        if (feedRes.ok && feedRes.data) {
          setDiscrepancies(feedRes.data.results || []);
        }
      } catch (err) {
  ...
  const reconciliationRate = summary?.reconciliation_rate != null ? summary.reconciliation_rate * 100 : 0;
  const openAnomalies = summary?.open_discrepancies_total ?? 0;
  const blockedTransactions = summary?.blocked_transactions ?? 0;
          ...
          <section className="stats-grid">
            {[
              ...
              },
              {
                ...
                icon: AlertTriangle,
                accent: 'amber',
                extra: String(blockedTransactions),
              },
              {
                label: 'System uptime (24h)',
                value: 99.98,
          ...
          <section className="chart-feed-grid">
            <div className="chart-panel">
              <div className="panel-header">
                ...
                <div className="panel-actions">
                  <div className="range-tabs">
                    <button className="range-button active">Live</button>
                  </div>
              ...
              <div className="chart-wrapper">
                <ResponsiveContainer width="100%" height={320}>
                  <LineChart data={trendData} margin={{ top: 20, right: 20, left: -16, bottom: 0 }}>
            ...
            <div className="feed-panel">
              ...
              <div className="feed-list">
                {loading ? (
                  <div className="feed-empty">Loading recent anomalies…</div>
                ) : discrepancies.length === 0 ? (
                  <div className="feed-empty">No recent discrepancies found.</div>
                ) : (
                  discrepancies.map((item) => (
                    <div key={item.id} className="feed-item">
                      <div>
                        <div className="feed-time">{item.detected_at ? new Date(item.detected_at).toLocaleString() : 'Unknown time'}</div>
                        <div className="feed-description">{item.details || item.anomaly_type}</div>
                      </div>
                      <div className="feed-meta">
                        <div className={`feed-status ${item.severity?.toLowerCase() || 'pending'}`}>
                          <span className="status-dot-small" />
                          {item.severity || 'Pending'}
                        </div>
                        <div className="feed-amount">{item.trans_id}</div>
                      </div>
                    </div>
                  ))
                )}
